...
const SpaceQuestionDetail = (props: LayoutSubFileProps) => {
  ...
  const { article, defaultValue } = useRequestFileInfo(fileGuid);

  const { isRenderList, fileList, myFileStats, total, fetchFileList, fileListLoading } =
  ...
  const { fileContentValue, GetApiArticlesGuidRequest, fileLoading } = useRequestFileBaseInfo();
  ...
  const GetApiQuestionsGuidReq = useRequestX(GetApiQuestionsGuid, {
    onSuccess: (res) => {
      if (res.data && res.data.myAnswerGuid && res.data.myAnswerGuid != '') {
        setAnswerGuid(res.data.myAnswerGuid);
        GetApiArticlesGuidRequest.run(res.data.myAnswerGuid);
      }

      if (res.data.myAnswerGuid == '' || res.data.myAnswerGuid == undefined) {
        setAnswerType(1);
    ...
    if (writingAnswer && answerType) {
      switch (answerType) {
        ...
        case 2:
          break;
        case 3:
          GetApiArticlesGuidRequest.run(subFileGuid);
          break;
      }
    }
  ...
  useEffect(() => {
    if (!fileContentValue) return;
    setAnswerContentValue(fileContentValue);
    console.log('useEffect: ', fileContentValue);
  }, [fileContentValue]);

  useEffect(() => {}, [subFileGuid]);
    ...
    <div className={styles.detail} id="detail">
      ...
      <div className={styles.question}>
        <div className={styles.question_header}>
          ...
          <div className={styles.question_header_footer}>{article && renderAnswerButton()}</div>
        </div>
      </div>
      {writingAnswer && (
        <div className={styles.answer_editor}>
          {answerType == 3 && (
            <Skeleton loading={fileLoading} active>
              <Editor
